...
import { getReturnRequests } from '../services/orderService';
...
const ReturnRequestsList = () => {
  ...
  const fetchReturnRequests = async (page = 1, search = '', status = '') => {
    setLoading(true);
    setError('');
    try {
  ...
  useEffect(() => {
    fetchReturnRequests(currentPage, searchTerm, statusFilter);
  }, [currentPage]);

  const handleSearch = (e) => {
    e.preventDefault();
    setCurrentPage(1);
    fetchReturnRequests(1, searchTerm, statusFilter);
  };

  const handleStatusFilter = (status) => {
    setStatusFilter(status);
    setCurrentPage(1);
    fetchReturnRequests(1, searchTerm, status);
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Manage and review return requests
            {pagination.total > 0 && (
      ...
      {loading ? (
        ...
        <div className="card text-center py-16">
          ...
          <p className="text-gray-500 mb-6">
            {searchTerm || statusFilter
              ? `No return requests match your filters. Try adjusting your search criteria.`
                    ...
                    <tr key={rr.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <div className="flex items-center">
                          {rr.product_image && (
                            <img
                              ...
                              src={getImageUrl(rr.product_image)}
                              alt={rr.product_title}
                              onError={(e) => {
                          ...
                          <div className="text-sm">
                            <div className="font-medium text-gray-900 line-clamp-1">
                              {rr.product_title || 'N/A'}
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{rr.buyer_name || 'N/A'}</div>
                        <div className="text-sm text-gray-500">{rr.buyer_email || ''}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900">{rr.reason_name || 'N/A'}</div>
                        {rr.description && (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-semibold text-gray-900">
                          {formatPrice(rr.price)}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {getReturnStatusBadge(rr.return_status)}
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button
                          onClick={() => navigate(`/return-requests/${rr.id}`)}
          ...
          {pagination.totalPages > 1 && (
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-8 pt-6 border-t border-gray-200">
              <div className="text-sm text-gray-600">
                ...
                of <span className="font-medium text-gray-900">{pagination.total}</span> requests
              </div>
